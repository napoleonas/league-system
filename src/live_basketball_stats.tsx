...
export default function LKLSystem() {
              ...
              <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
                {teamPlayers.map(player => (
                  <div key={player.id} className="border border-gray-200 rounded-lg p-4 hover:shadow-lg transition">
                    <div className="flex items-center justify-between mb-3">
                      ...
                      {isLoggedIn && userRole === 'admin' && (
                        <div className="flex gap-1">
                          <button onClick={() => handleEditPlayer(player)} className="p-1 bg-blue-100 hover:bg-blue-200 text-blue-600 rounded transition">
                            <Edit size={14} />
                          </button>
                          <button onClick={() => handleDeletePlayer(player.id)} className="p-1 bg-red-100 hover:bg-red-200 text-red-600 rounded transition">
                            <Trash2 size={14} />
                          </button>
                        </div>
                      )}
                    ...
                    <div className="grid grid-cols-3 gap-2 pt-3 border-t border-gray-100">
                      <div className="text-center">
                        <div className="font-bold text-orange-600">{player.ppg}</div>
                        <div className="text-xs text-gray-500">PPG</div>
                      </div>
                      <div className="text-center">
                        <div className="font-bold text-blue-600">{player.rpg}</div>
                        <div className="text-xs text-gray-500">RPG</div>
                      </div>
                      <div className="text-center">
                        <div className="font-bold text-green-600">{player.apg}</div>
                        <div className="text-xs text-gray-500">APG</div>
                      </div>
                    </div>
                    <button onClick={() => { setSelectedPlayer(player); setCurrentPage('playerDetail'); }} className="w-full mt-3 bg-gray-100 hover:bg-gray-200 text-gray-800 py-2 rounded-lg text-sm font-semibold transition">
                      Profilis →
                    </button>
                  </div>
                ))}
  ...
  const PlayerDetailPage = () => {
    ...
    return (
      <div className="space-y-6">
        <button onClick={() => setCurrentPage('players')} className="text-blue-600 hover:text-blue-700 flex items-center gap-2">
          <ChevronRight size={20} className="rotate-180" />
          Grįžti į žaidėjus
        </button>
        ...
        <div className="bg-white rounded-xl shadow-lg overflow-hidden">
          <div className="h-48 bg-gradient-to-br from-blue-600 to-purple-600 relative">
            <div className="absolute inset-0 flex items-center justify-center">
              <div className="text-9xl opacity-30">{selectedPlayer.nationality}</div>
            </div>
          </div>
          ...
          <div className="p-6">
            <div className="flex items-start justify-between mb-6">
              <div>
                <h2 className="text-4xl font-bold text-gray-800 mb-2">{selectedPlayer.name}</h2>
              ...
              {isLoggedIn && userRole === 'admin' && (
                <button onClick={() => handleEditPlayer(selectedPlayer)} className="flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg font-semibold transition">
                  <Edit size={18} /> Redaguoti
                </button>
              )}
            </div>
            ...
            <div className="grid md:grid-cols-4 gap-4 mb-6">
              <div className="bg-gray-50 rounded-lg p-4">
                <div className="text-sm text-gray-500 mb-1">Ūgis</div>
                <div className="text-2xl font-bold text-gray-800">{selectedPlayer.height} cm</div>
              </div>
              <div className="bg-gray-50 rounded-lg p-4">
                <div className="text-sm text-gray-500 mb-1">Svoris</div>
                <div className="text-2xl font-bold text-gray-800">{selectedPlayer.weight} kg</div>
              </div>
              <div className="bg-gray-50 rounded-lg p-4">
                <div className="text-sm text-gray-500 mb-1">Gimė</div>
                <div className="text-lg font-bold text-gray-800">{selectedPlayer.birthDate}</div>
              </div>
              <div className="bg-gray-50 rounded-lg p-4">
                <div className="text-sm text-gray-500 mb-1">Pilietybė</div>
                <div className="text-3xl">{selectedPlayer.nationality}</div>
              </div>
            </div>

            <div className="mb-6">
              <h3 className="text-xl font-bold text-gray-800 mb-3">Sezoninė statistika</h3>
              <div className="grid md:grid-cols-5 gap-4">
                <div className="bg-gradient-to-br from-orange-50 to-orange-100 rounded-lg p-4 text-center">
                  <div className="text-3xl font-bold text-orange-600">{selectedPlayer.ppg}</div>
                  <div className="text-sm text-gray-600">Taškai per rungtynes</div>
                </div>
                <div className="bg-gradient-to-br from-blue-50 to-blue-100 rounded-lg p-4 text-center">
                  <div className="text-3xl font-bold text-blue-600">{selectedPlayer.rpg}</div>
                  <div className="text-sm text-gray-600">Kamuoliai per rungtynes</div>
                </div>
                <div className="bg-gradient-to-br from-green-50 to-green-100 rounded-lg p-4 text-center">
                  <div className="text-3xl font-bold text-green-600">{selectedPlayer.apg}</div>
                  <div className="text-sm text-gray-600">Perdavimai per rungtynes</div>
                </div>
                <div className="bg-gradient-to-br from-purple-50 to-purple-100 rounded-lg p-4 text-center">
                  <div className="text-3xl font-bold text-purple-600">{selectedPlayer.fgPct}%</div>
                  <div className="text-sm text-gray-600">Metimų taiklumas</div>
                </div>
                <div className="bg-gradient-to-br from-yellow-50 to-yellow-100 rounded-lg p-4 text-center">
                  <div className="text-3xl font-bold text-yellow-600">{selectedPlayer.threePct}%</div>
                  <div className="text-sm text-gray-600">3-taškių taiklumas</div>
                </div>
              </div>
            </div>
            ...
            {selectedPlayer.bio && (
              <div>
                <h3 className="text-xl font-bold text-gray-800 mb-3">Biografija</h3>
    ...
    <div className="space-y-6">
      <div className="bg-white rounded-xl shadow-lg p-6">
        ...
        <div className="space-y-6">
          <div>
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-xl font-bold text-gray-800">Komandų valdymas</h3>
              <button onClick={handleAddTeam} className="flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg font-semibold transition">
                <Plus size={20} /> Pridėti komandą
              </button>
            </div>
            <div className="grid md:grid-cols-2 gap-4">
              {teams.map(team => (
                <div key={team.id} className="border border-gray-200 rounded-lg p-4 flex items-center justify-between">
                  ...
                  <div className="flex gap-2">
                    <button onClick={() => handleEditTeam(team)} className="p-2 bg-blue-100 hover:bg-blue-200 text-blue-600 rounded-lg transition">
                      <Edit size={18} />
                    </button>
                    <button onClick={() => handleDeleteTeam(team.id)} className="p-2 bg-red-100 hover:bg-red-200 text-red-600 rounded-lg transition">
                      <Trash2 size={18} />
                    </button>
                  </div>
                </div>
  ...
  const StatsPanelPage = () => (
    <div className="space-y-6">
      <div className="bg-white rounded-xl shadow-lg p-6">
        <h2 className="text-3xl font-bold text-gray-800 mb-6 flex items-center gap-3">
          <Activity size={32} className="text-green-600" />
          Live Statistikos Panelė
        </h2>

        {!liveGame ? (
          <div>
            <p className="text-gray-600 mb-6">Pasirinkite rungtynes, kurioms vesti statistiką:</p>
            <div className="grid md:grid-cols-2 gap-4">
              {teams.map((homeTeam, idx) => 
                teams.slice(idx + 1).map(awayTeam => (
                  <button
                    key={`${homeTeam.id}-${awayTeam.id}`}
                    onClick={() => startLiveGame(homeTeam.id, awayTeam.id)}
